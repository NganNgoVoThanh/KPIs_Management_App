...
// Import notification service sau để tránh circular dependency
...
class KpiService {
  ...
  private approveKpi(
    ...
    if (level === 1) {
      // Move to Head of Department
      nextStatus = 'PENDING_HOD'
      
      const hod = this.getNextApprover(user, 2)
      if (hod) {
        ...
        notifications.push(`Notification sent to ${hod.name}`)
      }
    } else if (level === 2) {
      // Move to BOD
      nextStatus = 'PENDING_BOD'
      
      const bod = this.getNextApprover(user, 3)
      if (bod) {
        const newApproval: Approval = {
          id: `approval-${generateUUID()}`,
          entityId: kpi.id,
        ...
        if (ns) {
          ns.createNotification(
            bod.id,
            'APPROVAL_REQUIRED',
            `KPI approval required (Final)`,
            { kpiId: kpi.id }
          )
        }
        notifications.push(`Notification sent to ${bod.name}`)
      }
    } else if (level === 3) {
      // Final approval
      nextStatus = 'APPROVED'
  ...
  private getNextApprover(currentUser: User, targetLevel: number): User | null {
    ...
    return {
      ...
      role: targetLevel === 1 ? 'LINE_MANAGER' : 
            targetLevel === 2 ? 'HEAD_OF_DEPT' : 'BOD',
