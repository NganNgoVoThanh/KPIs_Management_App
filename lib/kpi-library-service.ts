...
class KpiLibraryService {
  ...
  getEntries(filters?: {
    ...
    let entries: KpiLibraryEntry[] = JSON.parse(
      localStorage.getItem(this.STORAGE_KEY_ENTRIES) || '[]'
    );
  ...
  createUpload(data: {
    ...
    uploads.push(upload);
    localStorage.setItem(this.STORAGE_KEY_UPLOADS, JSON.stringify(uploads));
  ...
  getUploads(): KpiLibraryUpload[] {
    return JSON.parse(localStorage.getItem(this.STORAGE_KEY_UPLOADS) || '[]');
  }
  ...
  approveUpload(uploadId: string, comment?: string): void {
    ...
    if (!user || !['ADMIN', 'BOD'].includes(user.role)) {
      throw new Error('Admin or BOD access required');
    }
  ...
  getChangeRequests(filters?: {
    ...
    let requests: KpiLibraryChangeRequest[] = JSON.parse(
      localStorage.getItem(this.STORAGE_KEY_REQUESTS) || '[]'
    );
