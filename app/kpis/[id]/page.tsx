...
import type { KpiDefinition, Approval, ChangeRequest, KpiActual } from "@/lib/types"
...
export default function KpiDetailPage({ params }: PageProps) {
  ...
  const loadKpiData = () => {
    ...
    try {
      // Load KPI
      const kpiData = storageService.getKpiDefinitionById(unwrappedParams.id)
      if (!kpiData) {
        alert("KPI not found")
        router.push("/kpis")
        return
      ...
      // Load approvals
      const approvalsData = storageService.getApprovals(unwrappedParams.id, "KPI")
      setApprovals(approvalsData)

      // Load change requests
      const changeRequestsData = storageService.getChangeRequests({
        kpiDefinitionId: unwrappedParams.id
      })
      setChangeRequests(changeRequestsData)

      // Load actual if exists
      const actualsData = storageService.getKpiActuals({
        kpiDefinitionId: unwrappedParams.id
      })
      if (actualsData.length > 0) {
        setActual(actualsData[0])
      }
    } catch (error) {
  ...
  const handleEdit = () => {
    router.push(`/kpis/${unwrappedParams.id}/edit`)
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      storageService.deleteKpiDefinition(unwrappedParams.id)
  ...
  const handleRequestChange = () => {
    router.push(`/change-requests/create?kpiId=${unwrappedParams.id}`)
  }
