...
function KpisPageContent() {
  ...
  const handleExport = () => {
    try {
      // 1. Format data for Excel (Flatten structure)
      const exportData = kpis.map(kpi => ({
        ...
        'Unit': kpi.unit,
        'Weight (%)': kpi.weight,
        'Status': kpi.status ? kpi.status.replace(/_/g, ' ') : 'DRAFT',
        'Type': kpi.type ? kpi.type.replace('QUANT_', '').replace('_', ' ') : '',
        'Frequency': kpi.frequency || 'Quarterly',
        'Priority': kpi.priority || 'Medium',
        'SMART Score': kpi.smartScore || 0,
        'Created Date': kpi.createdAt ? new Date(kpi.createdAt).toLocaleDateString() : '',
      ...
      // 3. Auto-width columns (simple calculation)
      const wscols = Object.keys(exportData[0] || {}).map(k => ({ wch: 20 }));
      ws['!cols'] = wscols;

      // 4. Add worksheet to workbook
      XLSX.utils.book_append_sheet(wb, ws, "My KPIs")

      // 5. Save file
      XLSX.writeFile(wb, `KPIs_Export_${user?.name?.replace(/\s+/g, '_') || 'User'}_${new Date().toISOString().split('T')[0]}.xlsx`)

      toast({
        title: "Export Successful",
        description: "Your KPIs have been exported to Excel."
      })
    } catch (error) {
