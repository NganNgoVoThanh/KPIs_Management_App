// components/approvals/approval-card.tsx - FIXED: No mockdata, 2-level approval
"use client"
...
export function ApprovalCard({ kpi, onApprove, onReject, onViewDetails }: ApprovalCardProps) {
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      case "PENDING_LM":
        return "Pending Line Manager (Level 1)"
      case "PENDING_MANAGER":
        return "Pending Manager (Level 2)"
      case "APPROVED":
        return "Approved"
      case "REJECTED":
        return "Rejected"
    ...
    <Card className="hover:shadow-md transition-shadow">
          ...
          <div className="flex-1">
            ...
            <CardDescription className="flex items-center gap-4 text-sm">
              ...
              <span className="flex items-center gap-1">
                ...
                {kpi.submittedAt ? formatDistanceToNow(new Date(kpi.submittedAt), { addSuffix: true }) : "Not submitted"}
              </span>
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3">
          <h4 className="font-medium text-sm">Approval Progress (2 Levels)</h4>
